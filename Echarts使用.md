## Echarts建立可视化面板
	我们要学习对应的数据直观的让我们查看对应的数据项

### 使用的前端方法
- div 占位容器 
- css布局(这里采用的是less)
- 原生的javascript +  jquary 的使用(这里是使用了jQuery的包)
- rem的适配器(这个是需要在VSCode中下载的组件)
- *echarts*的使用和分析(大致讲三类：柱状图、饼状图、折线图)

### 方案制定
- 设计的是1920px的界面
1.利用flexible.js将屏幕分为24等份
2.cssrem 的插件的基准值为80px(这个就是需要在VSCode中下载的组件，并设置器值为80px)
注意：在安装cssrem时要要重启VSCode

### 基础的要求
- body 设置背景图，缩放为100%，行高为1.15
- css初始化
...
- 背景图在容器内显示
- 缩放比例为100%
- h1设置为标题，且为 白色(#fff) 行高为 80px (line-height：80px) 
- 时间模块 showTime 定位在右上角 right为30px  行高为75px 字体颜色为rgba(255, 255, 255, 0.7) =>(color： rgba(255, 255, 255, 0.7) ) 字体大小为20px(font-size：20px)
- 时间显示的格式：当前时间：xxxx年xx月xx-xx时xx分xx秒(这是是获取当前的时间)
...
### 公共面板模板 panel
- 高度为 310px
- 有一个为1像素的边框 (border:1px solid rgba(25, 186, 139, 0.17))
- 有一个为 背景图片
- padding为 上0 ，左右 15px ，下为 40px
- 下边距为 15px 
- 利用panel 盒子 before和after制作上面的两个小角 大小为 10px 线条为 2px solid #02a6b5
- 新增加盒子before 和 after 制作下侧的两个角 宽度高度为10px
...
```
### 柱状图 bar 模块(布局)
- 标签模块 h2 高度为48px 文字颜色为白色 文字大小为 20px
...
1.数字模块 no 有背景图颜色  rgba(101, 132, 226, 0.1); 有个15像素的内边距
...
3.旋转1  map2   大小为 643px 要加背景图片 因为要缩放100%    定位到中央  透明度  .6   做旋转动画   利用z-index压住球体
...
```
### Echarts的基础设置
了解每一个组件是什么每个组件的模块是干什么的

> 需要了解的主要配置：`series` `xAxis` `yAxis` `grid` `tooltip` `title` `legend` `color` 

- series
  - 系列列表。每个系列通过 `type` 决定自己的图表类型
  - 说白了：图标数据，指定什么类型的图标，可以多个图表重叠。
